...
def inference_loop(task_config: TaskConfig, num_inference_examples: int = None, num_completions: int = 1) -> None:
    ...
    for (batch, batch_idx) in tqdm(batched_tasks):
        thread_outputs = query_openai_over_inputs(batch, batch_idx, task_config=task_config, num_completions=num_completions)
    ...
    while i < n:
        ...
        try:
            ...
            if num_completions == 1:
                ...
                generated_answers = [
                    extract_answer_from_response(r, task_config) for r in all_responses
                ]
                res.update(
                    {
